...
exports.register = async (req, res) => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      User.create(
        { fullname, username, password, email, phone, address, role: useRole },
        (err, result) => {
          if (err) return reject(err);
          resolve(result);
        }
...
exports.login = (req, res) => {
  ...
  User.findUsername(username, async (err, result) => {
    ...
    if (user.role === "admin") {
      return res
        .status(200)
        .json({ message: "Login Admin!!!", token, redirect: "admin" });
    }

    //Fungsinya mengecek apakah peran (role) pengguna adalah 'superadmin' saat login. 
    //Jika peran tersebut adalah 'superadmin', maka akan mengembalikan respons dengan status 200 
    //dan JSON yang berisi pesan 'Login Super Admin!!!', dan token.
      ...
      return res
        .status(200)
        .json({ message: "Login", token, redirect: "user" });
    }
  });
};
